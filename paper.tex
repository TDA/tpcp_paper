...
Mitigation and recovery is of major concern when it comes to application and systems security. This area of research is important because recovery of a tainted system is the first concern post attack or exploitation.
...
A similar VM-based approach is taken in~\cite{oliveria2008b}. The Bezoar framework proposed in this paper recovers a system based on monitoring of propagation bytes in the system after hijacking attack has been detected. The framework replays the checkpointed run while ignoring inputs from malicious sources. Checkpointing methodolgy is also used in ExecRecorder[cite], which works on the idea of lod-based recovery. They propose a method to checkpoint system states and logging nondeterministic events. Recovery of the system is achieved by replaying the execution post attack and based on the log generated the system mitigates the attack execution during the reply.

Recovering programs from software bugs is also a method proposed for mitigation of software failures. Rx~\cite{qin2007} thoroughly explains a method based on checkpoiting to detect and recover from software bugs.  This framework detects software bugs in a program using sensors and once the bug is detected mitigation is done by changing the the execution environment dynamically. They have tabulated results displaying the mitigation of software bugs such as buffer overflow and stack overflow.
...
% needs sentence about importance of mitigation
This is because recovery of a tainted system is the first concern post attack or exploitation.

%% TODO: this needs more stuff about mitigation, theres nothing about mitigation here.
%% TODO: this para needs grammar-check: check usage of commas, colons, etc.
%% TODO: in order to --> to, repetitive word `tries'
%% TODO: expand VM at least first time of usage, abbreviate it after
...
%% TODO: this has passive voice, try to avoid it
